...
class DMD:
    ...
    def compute_havok_dmd(self,lamb=None):
        ...
        #print('havok: self.Vt_minus[:, :self.rank]', self.Vt_minus[:, :self.rank].shape)
        #print('self.Vt_plus[:, :self.rank]', self.Vt_plus[:, :self.rank].shape)
        #print('addition', (self.Vt_minus[:, :self.rank].T @ self.Vt_minus[:, :self.rank] + self.lamb*torch.eye(self.rank).to(self.device)).shape)
